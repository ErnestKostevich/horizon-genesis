...
- [x] **Self-correcting loop** — reflection no longer just self-grades and
  ...
  and the agent keeps going to close them (bounded by step budget +
  max 2 corrective rounds). Now fires in headless/CLI runs too.
- [x] **Memory that learns** — recall rewards the memories that actually
  surface in answers (`usefulness` score), so proven-useful context ranks
  higher over time. Zero behaviour change until a memory earns it.
- [x] **Semantic skill matching** — skills now match on meaning, not just
  keywords ("refactor React hooks" ≈ "convert class to functional"), with
  an offline-safe bag-of-words fallback.
- [x] **Eval/regression harness** — `npm run eval` replays golden agent
  tasks through the real loop with scored assertions, so quality changes
  are measurable and can't silently regress.
